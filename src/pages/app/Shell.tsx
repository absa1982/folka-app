  ...
  return (
    <aside style={{
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {D.nav.map((n) => (
          <NavLink key={n.id} to={`/app/${n.id}`} style={({ isActive }) => ({
            ...
            color: isActive ? 'var(--green-700)' : 'var(--text-body)',
            fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: isActive ? 700 : 500,
            transition: 'background var(--dur-fast)', textDecoration: 'none',
          })}>
            {({ isActive }) => (
              <>
                <Icon name={n.icon} />
                ...
                {n.badge && (
                  <span style={{
                    ...
                    background: isActive ? 'var(--green-600)' : 'var(--clay-500)', color: '#fff',
